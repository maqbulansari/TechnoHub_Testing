...
const AssessmentCandidate = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // Add admin-only fields if role is ADMIN
      if (role === "ADMIN") {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/assessment/update/${id}/`,
        payload,
        { headers: { Authorization: `Bearer ${accessToken}` } }
      );
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          {errors.trainer_score && (
            <p className="text-red-500 text-sm mt-1">{errors.trainer_score.message}</p>
          )}
        </div>
        ...
        <div>
          ...
          {errors.trainer_feedback && (
            <p className="text-red-500 text-sm mt-1">{errors.trainer_feedback.message}</p>
          )}
        </div>
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select value={field.value} onValueChange={field.onChange}>
                ...
                <SelectContent>
                  <SelectItem value="Selected">Yes</SelectItem>
                  <SelectItem value="Not">No</SelectItem>
                </SelectContent>
              </Select>
            )}
          />
          {errors.assessment_test_status && (
            <p className="text-red-500 text-sm mt-1">{errors.assessment_test_status.message}</p>
          )}
        </div>
